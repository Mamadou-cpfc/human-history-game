...
/* ===== 27 ===== */
{
  ...
  preText: (s) => {
    let t = "";
    if (s.A > 30) t += "国家は長期的な賭けを選んでいた。\n";
    if (s.D > 26) t += "挑戦は物語として人々を動かしていた。\n";
    if (s.timePressure > 28) t += "結果を急ぐ空気が漂っていた。\n";
    return t.trim();
  },
  ...
  choices: [
...

    {
      text: "問題から距離を取る",
      detail: "当面は内政に集中する。",
      effects: { bias_avoidance: 1, timePressure: +3 },
      next: 28
    }
...
/* ===== 28 ===== */
{
  ...
  preText: (s) => {
    let t = "";
    if (s.A > 32) t += "情報を整理する姿勢が保たれていた。\n";
    if (s.D > 28) t += "噂は意味として広がっていた。\n";
    return t.trim();
  },
...
/* ===== 29 ===== */
{
  ...
  preText: (s) => {
    let t = "";
    if (s.D > 30) t += "成功の物語が先行していた。\n";
    if (s.timePressure > 30) t += "判断を急ぐ圧力が高まっていた。\n";
    return t.trim();
  },
  ...
  choices: [
...

    {
      text: "介入を避ける",
      detail: "成り行きを見る。",
      effects: { bias_avoidance: 1, timePressure: +4 },
      next: 30
    }
...
/* ===== 30 ===== */
{
  ...
  preText: (s) => {
    let t = "";
    if (s.A > 36) t += "国家は長期的変化を受け入れ始めていた。\n";
    if (s.D > 32) t += "未知は恐れより期待として語られていた。\n";
    return t.trim();
  },
  ...
  choices: [
...

    {
      text: "宗教的使命として解釈する",
      detail: "意味づけで行動を統一する。",
      effects: { D: +3, B: +1, bias_narrative: 1 },
      next: 31
    },
...
/* ===== 56 ===== */
{
  ...
  preText: () =>
    "離脱と緊張を経て、\n" +
    "集団は元の状態には戻れなくなっていた。\n" +
    "選択は、もはや修正ではなく方向を決めるものになる。",
  text: () =>
    "ここから先、\n" +
    "全員が同じ形で存在し続けることはできない。\n" +
    "秩序は再構築されるか、\n" +
    "別の形へと変質する。",
  choices: [
    {
      text: "秩序を再定義する",
      detail: "新しい原則を明文化し、集団を再構成する。",
      effects: { A: +2, D: +1 },
      next: 57
    },
    {
      text: "強い統合を選ぶ",
      detail: "対立を抑え、決定を集中させる。",
      effects: { A: +3 },
      next: 57
    },
    {
      text: "意味を語り直す",
      detail: "なぜ続くのかを再び説明する。",
      effects: { bias_narrative: +2 },
      next: 57
    },
    {
      text: "成り行きに任せる",
      detail: "結果を受け入れる覚悟をする。",
      effects: { bias_avoidance: +2 },
      next: 57
    }
  ]
},
/* ===== 57 ===== */
{
  ...
  choices: [
    ...
    {
      text: "合意形成を重視する",
      detail: "時間をかけてでも広く納得を得る。",
      effects: { D: +1 },
      next: 58
    },
    {
      text: "象徴的統治を採る",
      detail: "理念と実務を分離し、安定を図る。",
      effects: { bias_narrative: +1 },
      next: 58
    },
    {
      text: "慣習に任せる",
      detail: "明文化を避け、曖昧さを残す。",
      effects: { bias_avoidance: +1 },
      next: 58
    }
  ]
